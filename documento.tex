...
Nos últimos tempos a linguagem JavaScript ganhou muita importância em quaisquer cenários que envolvam programação, e vem sendo utilizada em sites, aplicações, servidores, automação de testes, automação de tarefas, internet das coisas, entre outros. 

Fazendo parte das três principais tecnologias que movem a internet, sendo elas HTML, CSS e claro JavaScript, não é mais uma linguagem voltada somente para desenvolvedores front-end, mas sim uma obrigação para todo desenvolvedor, que tem como obrigação conhecer pelo menos o básico de JS. 

Este trabalho visa atualizar o leitor sobre as tendências do que mais vem sendo utilizado para a construção de aplicações utilizando a linguagem, mostrando os frameworks mais recentes e mais famosos. Mas lembre-se com grandes poderes vem grandes responsabilidades.

 \vspace{\onelineskip}

 \textbf{Palavras-chaves}: javascript, desenvolvimento de aplicações, typescript, frameworks
\end{resumo}
...
In recent times the JavaScript language has gained a lot of importance in any scenarios involving programming, and has been used in websites, applications, servers, automation of tests, automation of tasks, internet of things, among others.

Being part of the three main technologies that move the internet, being HTML, CSS and JavaScript, is no longer a language aimed only at front-end developers, but an obligation for every developer, who has to must know at least the basics of JS.

This work aims to update the reader on the trends of the most used for the construction of applications using the language, showing the most recent and most famous frameworks. But remember with great powers comes great responsibilities.

   \vspace{\onelineskip}
 
   \noindent 
   \textbf{Key-words}: javascript, application development, typescript, frameworks
 \end{otherlanguage*}
\end{resumo}
% ---
...
%   \item[abnTeX] ABsurdas Normas para TeX
  \item[JS] JavaScript
  \item[HTML] Abreviação para HyperText Markup Language, que em português significa Linguagem de Marcação de Hipertexto.
  \item[DOM] Abreviação para Document Object Model, que em português significa Modelo de Objetos e Documentos.
  \item[CSS] Abreviação para Cascading Style Sheets, que em português significa Folhas de Estilo em Cascata.
  \item[ECMA] Acrônimo para European Computer Manufacturers Association, que em português significa Associação Européia de Fabricantes de Computadores.
\end{siglas}
% ---
...
Atualmente, é praticamente impossível imaginar a internet sem a existência do JavaScript, certamente você já se deparou com alguns dos exemplos abaixo que são exemplos de aplicação da linguagem em \textit{web sites}.

\begin{figure}[h]
    \centering
    \label{fig:Monocristalino}
    \includegraphics[scale=0.25]{js-example-carrousel} \\
    \caption{\textit{Carrousel} - Um componente de apresentação de slides para percorrer imagens ou slides de texto - como um carrossel.}
...
\begin{figure}[h]
    \centering
    \label{fig:Monocristalino}
    \includegraphics[scale=0.25]{js-example-validation} \\
    \caption{\textit{Validation} - Validação de formulários}
...
\begin{figure}[h]
    \centering
    \label{fig:Monocristalino}
    \includegraphics[scale=0.25]{js-example-modal} \\
    \caption{\textit{Modal} - Caixas de diálogo para notificações ao usuário.}
...
\begin{figure}[h]
    \centering
    \label{fig:Monocristalino}
    \includegraphics[scale=0.5]{brendan-eich} \\
    \caption{\textit{Brendan Eich} - Criador do JavaScript.}
    Fonte: {Wikipedia (https://pt.wikipedia.org/wiki/)}
...
Conforme a linguagem evoluiu se tornou mais poderosa e independente do navegador. Isso possibilitou que a linguagem fosse utilizada não somente para a web como \textit{client side} mas agora em vários lugares. 
...
Praticamente tudo que envolve programação o JavaScript está presente, se eu quiser criar um software desktop posso utilizar por exemplo o \textit{framework} Electron (https://electronjs.org), desenvolvido pelo GitHub ele possibilita criar aplicativos desktop multiplataforma através do JavaScript. Caso meu desejo seja o desenvolvimento de jogos a Unity (https://unity3d.com/pt) é uma das plataformas que oferecem suporte a linguagem.

Hoje grandes empresas como Google, Microsoft, Netflix, Uber e Linkedin usam JavaScript até mesmo no \textit{back-end}. 

Isso acabou impactando o mundo dos desenvolvedroes, fazendo com que seja obrigatório todo programador saber pelo menos o básico da linguagem, mesmo atuanda na área de \textit{back-end} ou até mesmo de teste.

\section{ECMA}

Atualmente a linguagem se encontra na 8ª edição chamada de ECMAScript 2017.
...
Em seu site (http://www.ecma-international.org/publications/standards/Ecma-262.htm) é possível encontrar informações como versão atual e a documentação da linguagem, porém aconselho a buscar documentações na internet, uma boa referência a seguir é o site da Mozilla (https://developer.mozilla.org/pt-BR/docs/Web/JavaScript), ou também o site da W3Schools (https://www.w3schools.com/jsref/default.asp). 
...
% ----------------------------------------------------------
% ---
% Capitulo Introdução as estruturas
% ---
\chapter{Introdução as estruturas}
